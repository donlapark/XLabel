...
def main():
    ...
    if "database" in _state:
        if (filename[-5:] == '.xlsx') or (filename[-4:] == '.xls'):
            data = convert_to_downloadable(_state.database, 'excel')
            mime = "application/vnd.ms-excel"
        elif filename[-4:] == '.csv':
            data = convert_to_downloadable(_state.database, 'csv')
            mime = "text/csv"
...
def convert_to_downloadable(data, file_type):
    ...
    file_type: A target format for the conversion: 'csv' or 'excel'.
...
def update_and_save(label):
    ...
    with open('saved_models.pickle', 'wb') as _file:
